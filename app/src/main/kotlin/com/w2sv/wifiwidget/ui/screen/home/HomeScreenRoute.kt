...
import com.w2sv.wifiwidget.ui.sharedstate.location.OnLocationAccessGrant.TriggerWidgetDataRefresh
...
fun HomeScreenRoute(viewModel: HomeScreenViewModel = hiltViewModel()) {
    val locationPermissionCapability = LocalLocationAccessCapability.current
    val wifiState by viewModel.wifiState.collectAsStateWithLifecycle()
    val themeController = rememberThemeController()

    OnChange(locationPermissionCapability.foregroundPermissionsGranted) {
    ...
    ShowSnackbarOnWidgetPin(
        ...
        anyLocationAccessRequiringPropertyEnabled = { viewModel.anyLocationAccessRequiringPropertyEnabled }
    )
            ...
            when {
                ...
                anyLocationAccessRequiringPropertyEnabled() && !locationAccess.foregroundPermissionsGranted -> AppSnackbarVisuals(
                    ...
                    action = SnackbarAction(
                        ...
                        callback = {
                            locationAccess.requestPermission(onGrant = TriggerWidgetDataRefresh)
                        }
                    )
                )
